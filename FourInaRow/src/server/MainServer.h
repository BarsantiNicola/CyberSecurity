...
namespace server {
    ...
    class MainServer {
        ...
        private:
            ...
            Message* loginHandler( Message* message,  int socket, int* nonce );                     //  MANAGES MESSAGE LOGIN_REQ
            Message* keyExchangeHandler( Message* message , string username, int* nonce );          //  MANAGES MESSAGE KEY_EXCHANGE
            Message* userListHandler( Message* message, string username, int* nonce );              //  MANAGES MESSAGE USER_LIST_REQ
            Message* rankListHandler( Message* message, string username, int* nonce );              //  MANAGES MESSAGE RANK_LIST_REQ
            Message* logoutHandler( Message* message , string username , int socket, int* nonce );  //  MANAGES MESSAGE LOGOUT_REQ
            Message* matchHandler( Message* message, string username, int* nonce );                 //  MANAGES MESSAGE MATCH
            Message* acceptHandler( Message* message , string username, int* nonce );               //  MANAGES MESSAGE ACCEPT
            Message* rejectHandler( Message* message , string username, int* nonce );               //  MANAGES MESSAGE REJECT
            Message* withdrawHandler( Message* message, string username, int* nonce );              //  MANAGES MESSAGE WITHDRAW_REQ
            Message* disconnectHandler( Message* message, string username, int* nonce );            //  MANAGES MESSAGE DISCONNECT
            Message* gameHandler( Message* message, string username, int* nonce );                  //  MANAGES MESSAGE GAME
